...
def parse_arguments():
    ...
    parser.add_argument('--loss', default='focal', type=str, help='loss type')
    parser.add_argument('--multitask', default=True, type=lambda x: (str(x).lower() == 'true'), help='use multitask structure or not')    
    parser.add_argument('--jyutping', default=True, type=lambda x: (str(x).lower() == 'true'), help='use jyutping embedding or not')
